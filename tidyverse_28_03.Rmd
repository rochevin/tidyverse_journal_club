...
```{r show1, echo=F}
kable(treatments,caption = "Typical presentation dataset",booktabs = T) %>%kable_styling(latex_options = "striped",position = "center")
```
...
\begin{center}
```{r, echo=F}
kable(treatments.2,caption = "The same data but structured differently",booktabs = T) %>%kable_styling(latex_options = "striped",position = "center")
```
\end{center}
...
treatments.3 %>%
    mutate(treatment = cell_spec(treatment, align = "c",background =factor(treatment,unique(treatments.3$treatment),RColorBrewer::brewer.pal(length(unique(treatments.3$treatment)), "Spectral")))) %>%
    mutate(person = cell_spec(person, align = "c",background =factor(person,unique(treatments.3$person),RColorBrewer::brewer.pal(length(unique(treatments.3$treatment)), "Set1")))) %>% 
    mutate(result = cell_spec(result, align = "c",background ="black",color = "white")) %>% kable(caption = "An exemple of a tidy dataset",escape=F) %>%kable_styling(position = "center")
```


```{r boxplot3, echo=TRUE, fig.height=2.8, fig.width=8, message=FALSE, warning=FALSE}
...
iris.tbl %>% head() %>% kable(caption = "iris dataset",booktabs = T) %>%kable_styling(latex_options = "striped",position = "center")
```
...
```
```{r}
iris.meta %>% kable(caption = "Metadata for iris dataset",booktabs = T) %>%kable_styling(latex_options = "striped",position = "center")
```
...
```{r}
iris.meta %>% kable(caption = "Metadata for iris dataset",booktabs = T) %>%kable_styling(latex_options = "striped",position = "center") %>% row_spec(1, strikeout = T)
```
...
\small
```{r}
iris.tbl %>% group_by(Species) %>%slice(1:2) %>% kable(caption = "iris dataset",booktabs = T) %>%kable_styling(latex_options = "striped",position = "center") %>% row_spec(1:2,strikeout = T)
```
...
iris.tbl %>% filter(Species != "virginica") %>%
    full_join(iris.meta.nosetosa,by = "Species")
```
...
treatment.meta %>% kable("latex",caption = "Patients files",booktabs = T) %>%kable_styling(latex_options = "striped",position = "center")
```
...
* __ggpubr__ \& __ ggstatsplot__: Publication ready plots \& add statistical tests.
