...
\begin{center}
```{r show1, echo=F}
kable(treatments,"latex",caption = "Typical presentation dataset")
```
\end{center}
...
\begin{center}
```{r, echo=F}
kable(treatments.2,"latex",caption = "The same data but structured differently")
```
\end{center}
...
kable(treatments.3,"latex",caption = "An exemple of a tidy dataset")
```
\end{center}
...
iris.tbl %>% head() %>% kable(format = "latex",caption = "iris dataset")
```
\end{center}
...
```
```{r}
iris.meta %>% kable("latex",caption = "Metadata for iris dataset")
```
...
```{r echo=TRUE,eval=F}
iris.meta.nosetosa <- iris.meta %>% filter(Species !="setosa")
iris.tbl %>% filter(Species != "virginica") %>% full_join(iris.meta.nosetosa,by = "Species") %>% print(n=3)
```
...
treatment.meta %>% kable("latex",caption = "Patients files")
```
...
```{r echo=TRUE, fig.height=5, fig.width=10, message=FALSE, warning=FALSE,eval=F}
p <- iris.tbl %>%# Data
...
```{r echo=F, fig.height=12, fig.width=22, message=FALSE, warning=FALSE}
